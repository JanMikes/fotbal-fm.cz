...
const { mapMatchResult, safeMapMatchResult, safeMapMatchResults } = await import(
  '../../../../infrastructure/strapi/mappers/match-result'
);
...
describe('mapMatchResult', () => {
  it('maps valid raw data to MatchResult', () => {
    const result = mapMatchResult(makeValidRaw());
  ...
  it('uses createdAt date when matchDate is null', () => {
    const result = mapMatchResult(makeValidRaw({ matchDate: null }));
    expect(result.matchDate).toBe('2025-03-15');
  });

  it('handles null categories', () => {
    const result = mapMatchResult(makeValidRaw({ categories: null }));
    expect(result.categories).toEqual([]);
  });

  it('throws ValidationError for invalid data', () => {
    expect(() => mapMatchResult({ invalid: 'data' })).toThrow();
  });

  it('throws ValidationError when required fields are missing', () => {
    expect(() => mapMatchResult({ id: 1 })).toThrow();
  });
});

describe('safeMapMatchResult', () => {
  it('returns mapped result for valid data', () => {
    const result = safeMapMatchResult(makeValidRaw());
    expect(result).not.toBeNull();
    expect(result?.homeTeam).toBe('FK Frýdek-Místek');
  });

  it('returns null for invalid data', () => {
    const result = safeMapMatchResult({ invalid: 'data' });
    expect(result).toBeNull();
  });
});

describe('safeMapMatchResults', () => {
  it('maps valid items and filters invalid ones', () => {
    const results = safeMapMatchResults([
      makeValidRaw(),
      { invalid: 'data' },
  ...
  it('returns empty array when all items are invalid', () => {
    const results = safeMapMatchResults([{ bad: true }, { also: 'bad' }]);
