...
export const GET = withAuth(async (
  ...
  // Path is /api/tournaments/[id]/comments, so id is at index -2
  const id = pathParts[pathParts.length - 2];

  addApiBreadcrumb('Getting tournament comments', { id });

  const service = CommentService.forUser(jwt);
  const result = await service.getByEntity('tournament', id);
